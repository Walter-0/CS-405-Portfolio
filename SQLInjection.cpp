...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records) {
    ...
    // This regular expression checks for a case insensitive SQL injection by searching for:
    // the word "or" surrounded by a whitespace character (\\sor\\s)
    // an optional single quote [']?
    // any number of alphanumeric characters [A-Za-z0-9]*
    // an optional single quote [']?
    // an equals sign =
    // an optional single quote [']?
    // any number of alphanumeric characters [A-Za-z0-9]*
    // an optional single quote [']?
    // If a SQL injection is detected, the user is notified, and run_query returns false, which prevents it from running.
    if (std::regex_search(sql , std::regex("(\\sor\\s)[']?[A-Za-z0-9]*[']?=[']?[A-Za-z0-9]*[']?", std::regex_constants::icase))) {
        std::cout << "SQL injection detected (OR value=value). Aborting query" << std::endl;
        return false;
    }
