...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps) {
  ...
  for (unsigned long int i = 0; i < steps; ++i) {
    ...
    // If the result is less than zero or the increment value, that indicates an overflow.
    // Additionally, an infinity result indicates an overflow
    if (result < 0 || result < increment || std::isinf(result)) {
      return 'X';
    }
  }
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps) {
  ...
  // If initial value is less than the total subtracted value, then an overflow will happen
  if (start < (decrement * steps)) {
    return 'X';
  }

  for (unsigned long int i = 0; i < steps; ++i) {
    // Add tempResult variable to check value of result before and after subtraction
    T tempResult = result;
    result -= decrement;

    // If result becomes greater than tempResult, an overflow occurred.
    // If result is less than zero, than an overflow occurred.
    if (tempResult < result  || result < 0) {
      return 'X';
    }
  }
...
void test_overflow() {
  ...
  // Check for the char value 'X' and print out an overflow indication if it happened
  if (result == 'X') {
    std::cout << "The operation has resulted in a numeric overflow. Please try again with different inputs." << std::endl;
  } else {
    std::cout << +result << std::endl;
  }
}
